...
conduct_transfer_entropy_analysis <- function(data) {
  cat("Conducting Transfer Entropy analysis...\n")
  
  te_variables <- c("lnCO2", "lnUR", "lnURF", "lnURM", "lnPCGDP", "lnTrade", "lnRES")
  
  ts_data <- data %>%
    dplyr::select(country, year, country_code, dplyr::all_of(te_variables)) %>%
  ...
  for(i in 1:n_vars) {
    ...
    cat("Completed variable", i, "of", n_vars, "\n")
  }
  
  # Ensure we have meaningful relationships
  if(max(te_matrix) == 0) {
    cat("No transfer entropy detected, using enhanced correlation-based network...\n")
  ...
  if(sum(te_adj > 0) == 0) {
    cat("No edges above threshold, lowering threshold...\n")
...
create_te_based_instruments <- function(data, te_results) {
  cat("Creating instruments based on Transfer Entropy causal discovery...\n")
